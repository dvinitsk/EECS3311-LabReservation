...
public class ManagerAITest {

	private Manager user;

	@Before
	public void setUp() throws Exception {
		user=new Manager("Id123","John Doe",new Credentials("a@email.com", "Pass123"),"Lassonde");
	...
	public void testDepartment() {
		assertEquals("Lassonde", user.getDepartment());
		user.setDepartment("Chemistry");
		assertEquals("Chemistry", user.getDepartment());
	}
	
	@Test
	public void modifyOperationalStatus() {
		Equipment e= new Equipment("EQ-01", "Microscope", "BRG 313");
		user.enableEquipment(e);
		assertEquals(OperationalStatus.AVAILABLE, e.getOperationalStatus());
		user.disableEquipment(e);
		assertEquals(OperationalStatus.DISABLED, e.getOperationalStatus());
		user.markMaintenance(e);
		assertEquals(OperationalStatus.MAINTENANCE, e.getOperationalStatus());
	}
